...
async function fetchUserConfigLogs(
    ...
    try {
        while (true) {
            ...
            console.log(`[AUDIT CONFIGURATION] Loading Page ${page}...`);
        ...
        for (const log of rawRecords) {
            const entries = processUserConfigLog(
                log,
                userMapping,
                subaccountName
            );
        ...
        throw new Error(
            `Failed to fetch configuration audit logs for ${subaccountName || "Unknown"}` +
            `${status ? ` (HTTP ${status})` : ""}: ${details}`
        );
    }
}
...
function deduplicateUserAuditEntries(entries) {
    ...
    for (const entry of entries) {
        const timestamp = getSecondPrecisionTimestamp(entry.timestamp);
...
/**
 * Consolidates multiple audit records for the exact same user persona into a single entry.
 * Merges role collections and user roles while preserving the latest activity details.
 */
function consolidateUserPersonaRecords(entries) {
    if (!Array.isArray(entries) || entries.length === 0) {
        return [];
    }
    ...
    for (const entry of entries) {
        ...
        const compositeKey = `${subaccount}|${userId}|${eventType}|${event}`;

        // Ensure entry object has a primary key ID
        if (!entry.ID) {
            entry.ID = cds.utils.uuid();
        }

        if (!personaMap.has(compositeKey)) {
            personaMap.set(compositeKey, entry);
            continue;
        }

        const existingEntry = personaMap.get(compositeKey);

        const currentTime = new Date(entry.time || entry.timestamp || 0).getTime();
        const existingTime = new Date(existingEntry.time || existingEntry.timestamp || 0).getTime();

        // Preserve existing ID if updating with newer timestamp data to allow database UPSERT
